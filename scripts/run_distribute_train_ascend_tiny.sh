...
if [ $# != 4 ] && [ $# != 1 ]
then
    echo "Usage: bash run_distribute_train_tiny.sh [CONFIG_PATH] [DATA_PATH] [HYP_PATH] [RANK_TABLE_FILE]"
    echo "OR"
    echo "Usage: bash run_distribute_train_tiny.sh [RANK_TABLE_FILE]"
exit 1
fi
...
then
  ...
  CONFIG_PATH=$"./config/network_yolov7/yolov7-tiny.yaml"
  DATA_PATH=$"./config/data/coco.yaml"
  HYP_PATH=$"./config/data/hyp.scratch.tiny.yaml"
fi
