...
from .general import segments2boxes, xywhn2xyxy, xyxy2xywh
from .augumentations import load_image, load_mosaic, load_mosaic9, letterbox, random_perspective, \
...
class LoadImagesAndLabels:  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 single_cls=False, stride=32, pad=0.0, max_box_per_img=160, prefix=''):
...
def create_dataloader(path, imgsz, batch_size, stride, opt, epoch_size=300,
                      augment=False, pad=0.0, rect=False, rank=0, rank_size=1,
                      num_parallel_workers=8, shuffle=True, drop_remainder=True,
                      max_box_per_img=160, prefix=''):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    dataset = LoadImagesAndLabels(path, imgsz, batch_size,
                                  augment=augment,  # augment images
                                  hyp=opt,  # augmentation hyperparameters
    ...
    ds = ds.batch(batch_size,
                  per_batch_map=LoadImagesAndLabels.collate_fn,
...
if __name__ == '__main__':
    ...
    ds, _, ds_size = create_dataloader(train_path, imgsz, batch_size, gs, opt, epoch_size=1,
                                       ...
                                       rank=0, rank_size=1, num_parallel_workers=8,
